...
@dataclass
class ShoppingItem:
    """One line on this week's shopping list (config/shopping_list.yaml).

    Generated by `python -m grocery_scraper.select_items` from your catalog -
...
@dataclass
class CatalogItem:
    """One product in your master catalog (config/catalog.yaml) - everything
    you might buy, not just this week's list. select_items.py lets you pick a
    subset + quantities from this to generate shopping_list.yaml."""

    name: str
    unit: str = "each"
    default_quantity: float = 1.0
    aliases: list[str] = field(default_factory=list)
    # Prices you've noted yourself for stores with no working scraper (e.g.
    # Iceland), keyed by store name. Update these whenever you shop there.
    manual_prices: dict = field(default_factory=dict)
...
class StoreConfig:
    name: str
    enabled: bool = True
    # True for stores with no working scraper - prices come from
    # config/catalog.yaml's manual_prices instead of an Apify actor.
    manual: bool = False
